...
if [ ! $DATABASENAME ]
    then
        DBARRAY=`psql $DATABASENAME -h "$HOSTNAME" -U "$USERNAME" -At -c "$FULL_BACKUP_QUERY" postgres`
    else
        DBARRAY=($DATABASENAME)
fi;
...
do
	...
	then
		...
		if ! pg_dump -Fc -h "$HOSTNAME" -U "$USERNAME" "$DATABASE" -f $FINAL_BACKUP_DIR"$DATABASE".dump.in_progress; then
			echo "[!!ERROR!!] Failed to produce custom backup database $DATABASE" 1>&2
		else
			mv $FINAL_BACKUP_DIR"$DATABASE".dump.in_progress $FINAL_BACKUP_DIR"$DATABASE".dump
		fi
	fi
